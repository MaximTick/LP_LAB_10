...
{//0-99-системные ошибки 
	...
	//110-119-ошибки открытия и чтения фалов
	ERROR errors[ERROR_MAX_ENTRY] =	//таблица ошибок
	{
		ERROR_ENTRY(0, "Недопустимый код ошибки"),	//код ошибки вне диапазона
		...
		ERROR_ENTRY(110, "Ошибка при открытий файла с исходным кодом (-in)"),
	...
	ERROR geterrorin(int id, int line = -1, int col = -1)//функция с заданными параметрами
	{
		ERROR e;
		if (0 < id && id < ERROR_MAX_ENTRY)
		{
			e = errors[id];
			e.inext.line = line; //строка в которой ошибка
			e.inext.col = col; // позиция в которой находится ошибка
		}
		else
		{
			e = errors[0];
